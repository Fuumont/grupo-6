...
import { verificarPropuesta } from "../services/verificacion.service.js"
...
export async function getAprobacionService() {
  try {
    ...
    if (!aprobaciones || aprobaciones.length === 0)
      return [null, "No hay aprobaciones registradas"];
...
export async function getAprobacionByIdService(id) {
  try {
    const aprobacionRepository = AppDataSource.getRepository(AprobacionSchema);
    const aprobacion = await aprobacionRepository.findOne({
      where: { id: id },
      relations: ["usuario", "propuesta"],
    ...
    if (!aprobacion)
      return [null, "No hay aprobaciones registradas con este id"];
...
export async function createAprobacionService(body, user) {
  try {
    ...
    const propuesta = await propuestaRepository.findOne({
      where: { id: body.propuesta },
    });
    if (!propuesta || propuesta.length === 0)
      return [null, "La propuesta no existe"];

    // comprueba si el usuario ya creó un voto para la propuesta
    const alreadyVoted = await aprobacionRepository.findOne({
        where: {
            usuario: { id: user },
            propuesta: { id: body.propuesta }
        }
    });
    if (alreadyVoted) return [null, "El usuario ya ha votado por esta propuesta"];

    // finalmente crea el voto/aprobacion, pa q le puse ese nombre bruh
    ...
    await verificarPropuesta(body.propuesta); // llama a la funcion de verificacion
...
export async function updateAprobacionService(id, body) {
  try {
    ...
    if (!aprobacion)
      return [null, "No hay aprobaciones registradas con este id"];
    const aprobacionUpdated = await aprobacionRepository.save({
      ...aprobacion,
      ...body,
    });
    await verificarPropuesta(aprobacion.propuesta.id); // llama a la verificacion
